...
class Prompt_Builder_REST {
    public static function register_routes() {
        register_rest_route('prompt-builder/v1', '/generate', [
            'methods'  => 'POST',
        ...
        register_rest_route('prompt-builder/v1', '/create-draft', [
            'methods'  => 'POST',
            'callback' => [self::class, 'create_draft_post'],
            'permission_callback' => function () {
                return current_user_can('manage_options');
            }
        ]);
    ...
    public static function handle_generate($request) {
        ...
        if (!is_array($requisitos)) {
            return new WP_Error('invalid_requisitos', 'Requisitos inválidos.', ['status' => 400]);
        }
    ...
    public static function create_draft_post($request) {
        ...
        if (!$content) {
            return new WP_Error('empty_content', 'Conteúdo não pode estar vazio.', ['status' => 400]);
        }
        ...
        if (is_wp_error($post_id)) {
            return new WP_Error('insert_failed', 'Falha ao criar rascunho.', ['status' => 500]);
        }
